...
class Appointment extends Model {
  ...
  Appointment.fromJson(Map<String, dynamic> json) {
    ...
    emergency = stringFromJson(json, 'is_emergency');
  }
  ...
  double getCouponValue() {
    ...
    if (coupon == null || !(coupon?.hasData ?? false)) {
      ...
      if (coupon!.discountType == 'percent') {
        return -(total * (coupon!.discount ?? 0.0) / 100);
      } else {
        return -(coupon!.discount ?? 0.0);
      }
    }
  }
  ...
  double getSubtotal() {
    double total = 0.0;
    total = doctor?.getPrice ?? 0.0;
    return total;
  }

  bool get canAppointmentAtClinic {
    return this.doctor?.enableAtClinic ?? false;
  }

  bool get canOnlineConsultation{
    return this.doctor?.enableOnlineConsultation ?? false;
  }

  bool get canAppointmentAtCustomerAddress {
    return this.doctor?.enableAtCustomerAddress ?? false;
  }
  ...
  bool isLive() => clinic?.available ?? false;
}
