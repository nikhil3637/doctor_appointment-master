    ...
    return Obx(() {
      return Scaffold(
        appBar: AppBar(
          ...
          bottom: controller.faqSpecialities.isEmpty
              ? TabBarLoadingWidget()
              : TabBarWidget(
                  tag: 'help',
                  initialSelectedId: controller.faqSpecialities.elementAt(0).id,
                  tabs: List.generate(controller.faqSpecialities.length, (index) {
                    var _speciality = controller.faqSpecialities.elementAt(index);
                    return ChipWidget(
                      tag: 'help',
                      text: _speciality.name!.tr,
                      id: _speciality.id,
                      onSelected: (id) {
                        controller.getFaqs(specialityId: id);
                      },
                    );
                  }),
                ),
        ...
        body: RefreshIndicator(
          onRefresh: () async {
            ...
            controller.refreshFaqs(
              showMessage: true,
              specialityId: Get.find<TabBarController>(tag: '/help').selectedId.value,
            );
