      ...
      if (_appointment.value.status?.order == Get.find<GlobalService>().global.value.onTheWay) {
        ...
        return Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            if (_appointment.value.status?.order == Get.find<GlobalService>().global.value.accepted && _appointment.value.payment == null)
              Expanded(
            ...
            if ((_appointment.value.status?.order ?? false) == Get.find<GlobalService>().global.value.ready && (_appointment.value.online ?? false))

            if (_appointment.value.status?.order == Get.find<GlobalService>().global.value.ready && (_appointment.value.online ?? false))
            ...
            if ((_appointment.value.status?.order ?? false) == Get.find<GlobalService>().global.value.inProgress && !(_appointment.value.online ?? false))

            if (_appointment.value.status?.order == Get.find<GlobalService>().global.value.inProgress && !(_appointment.value.online ?? false))
            ...
            if ((_appointment.value.status?.order ?? 0) >= (Get.find<GlobalService>().global.value.done ?? 0) && _appointment.value.payment != null)
              Expanded(
                child: BlockButtonWidget(
                    text: Stack(
                      alignment: AlignmentDirectional.centerEnd,
            ...
            SizedBox(width: 10),
            if (!(_appointment.value.cancel ?? false) && (_appointment.value.status?.order ?? 0) < (Get.find<GlobalService>().global.value.onTheWay ?? 0))
              MaterialButton(
                onPressed: () {
