...
class AppointmentController extends GetxController {
  ...
  Future<void> getAppointment() async {
    try {
      ...
      if (appointment.value.status == Get.find<AppointmentsController>().getStatusByOrder(Get.find<GlobalService>().global.value.inProgress ?? 0) && timer == null) {
        timer = Timer.periodic(Duration(minutes: 1), (t) {
          appointment.update((val) {
            if(val?.duration == null) return;
  ...
  Future<void> startAppointment() async {
    try {
      final _status = Get.find<AppointmentsController>().getStatusByOrder(Get.find<GlobalService>().global.value.inProgress ?? 0);
      appointment.value.startAt = DateTime.now();
      appointment.value.status = _status;
      await _appointmentRepository.update(appointment.value);
      appointment.update((val) {
        val?.startAt = appointment.value.startAt;
        val?.status = _status;
      });
      timer = Timer.periodic(Duration(minutes: 1), (t) {
        appointment.update((val) {
  ...
  Future<void> finishAppointment() async {
    try {
      final _status = Get.find<AppointmentsController>().getStatusByOrder(Get.find<GlobalService>().global.value.done ?? 0);
      appointment.value.endsAt = DateTime.now();
      appointment.value.status = _status;
      final result = await _appointmentRepository.update(appointment.value);
      appointment.update((val) {
  ...
  Future<void> cancelAppointment() async {
    try {
      if ((appointment.value.status?.order ?? 0) < Get.find<GlobalService>().global.value.onTheWay!) {
