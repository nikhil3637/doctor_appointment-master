...
class AppointmentsController extends GetxController {
  ...
  Future loadAppointmentsOfStatus({String statusId = ''}) async {
    try {
      ...
      if (appointmentStatuses.isNotEmpty) {
        _appointments = await _appointmentsRepository.all(page: page.value);
      }
  ...
  Future<void> cancelAppointmentService(Appointment appointment) async {
    try {
      if ((appointment.status?.order ?? 0) < (Get.find<GlobalService>().global.value.onTheWay ?? 0)) {
        final _status = getStatusByOrder(Get.find<GlobalService>().global.value.failed ?? 0);
