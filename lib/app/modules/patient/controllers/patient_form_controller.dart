...
class PatientFormController extends GetxController {
  ...
  void updatePatientForm() async {
    ...
    if (patientForm.currentState?.validate() ?? false) {
      try {
        ...
        await _patientRepository.update(patient.value);
        Get.showSnackbar(
            Ui.SuccessSnackBar(message: "Patient saved successfully".tr));
  ...
  Future<void> deletePatient(Patient patient) async {
    try {
      ...
      Get.showSnackbar(
          Ui.SuccessSnackBar(message: "Patient deleted successfully".tr));
      Get.find<RootController>().changePage(3);
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
    }
