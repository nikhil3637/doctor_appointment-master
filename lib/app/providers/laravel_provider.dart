...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<Map<String, List<Slide>>> getHomeSlider() async {
    Uri _uri = getApiBaseUri("indexslide");
    Get.log(_uri.toString());
    var response = await _httpClient.getUri(_uri, options: _optionsCache);
    if (response.data['success'] == true) {
      try {
        return <String, List<Slide>>{
                'slide_top': response.data['data']['slide_top'].map<Slide>((obj) => Slide.fromJson(obj)).toList(),
                'slide_mid': response.data['data']['slide_mid'].map<Slide>((obj) => Slide.fromJson(obj)).toList(),
              };
      } catch (e) {
        print(e);
      }
  ...
  Future<List<Speciality>> getSubSpecialities(String specialityTypeId) async {
    final _queryParameters = {
      'search': "speciality_type:$specialityTypeId",
      'searchFields': "speciality_type:=",
      'orderBy': 'order',
      'sortBy': 'asc',
    };
    Uri _uri = getApiBaseUri("specialities").replace(queryParameters: _queryParameters);
  ...
  Future<List<Clinic>> getSpecialityClinicsWithPagination(String specialityId, int page) async {
    // final _address = Get.find<SettingsService>().address.value;
    // TODO get Only Recommended
   /* var _queryParameters = {
      'only': 'only=id;name;has_media;media;total_reviews;rate;address;distance;available;specialities&with=address;specialities&search=specialities.id:$specialityId&searchFields=specialities.id:=',
      'specialities&with': 'address;',
      'specialities&search': 'specialities.id:15',
      'searchFields': 'specialities.id:=',
      'limit': '10',
      'offset': ((page - 1) * 4).toString()
    };*/
    // Uri _uri = getApiBaseUri("clinics").replace(queryParameters: _queryParameters);
    Uri _uri = getApiBaseUri("clinics?"
        "only=id;name;has_media;media;total_reviews;rate;address;distance;available;"
        "specialities;doctoremr&with=address;specialities;doctoremr&search=specialities.id:$specialityId"
        "&searchFields=specialities.id:=&limit=10&offset=${((page - 1) * 4)}");
    Get.log(_uri.toString());
    var response = await _httpClient.getUri(_uri, options: _optionsCache);
    if (response.data['success'] == true) {
      return response.data['data'].map<Clinic>((obj) => Clinic.fromJson(obj)).toList();
  ...
  Future<Map<String, dynamic>> checkTimeSlot(Appointment appointment) async {
    var _queryParameters = {
      'api_token': authService.apiToken,
    };
    Uri _uri = getApiBaseUri("appointments/checktimeslot").replace(queryParameters: _queryParameters);
    var response = await _httpClient.postUri(
      _uri,
